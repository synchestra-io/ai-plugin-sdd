...
AND the user invokes `spec-studio:specify` on the clear-intent path with intent "I want a Feature that scores card transactions for fraud risk and rejects high-risk ones"
...
GIVEN the user invokes `spec-studio:specify` with an explicit Idea path (e.g., points the skill at `spec/ideas/payment-fraud.md`)
WHEN the skill processes the invocation
